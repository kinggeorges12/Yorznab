...
2. [Setup](#setup): Open the Yorznab dashboard and setup your apps.
...
wget -O ./app/docker-compose-latest.yml https://github.com/kinggeorges12/Yorznab/blob/main/docker-compose-latest.yml
...
Invoke-WebRequest -OutFile ".\app\docker-compose-latest.yml" -Uri "https://github.com/kinggeorges12/Yorznab/blob/main/docker-compose-latest.yml"
(Get-Content './app/docker-compose-latest.yml') -replace '/path/to/yorznab',$YORZNAB_DIR | Set-Content ./app/docker-compose-run.yml
docker compose -f ./app/docker-compose-run.yml up -d
```

# Setup
The Yorznab dashboard displays status information and connects your apps. Authenticate through the dashboard for the first-run to create the Login Passkey. Open a web browser with access to the server and point it at the base url of the Docker container, e.g., [`http://localhost:9116/`](http://localhost:9116/) or http://myserver.local:9116/.
...
7. Apply rules for the Indexer in Radarr and Sonarr apps to continue seeding after downloading to Seed Ratio, Seed Time, and Season-Pack Seed Time.
...
3. Run: Double-click the `start.bat` or `start.sh` (requires the run permission in Linux).
4. Startup [Optional]: Create a new entry in the Cron/Task Scheduler with the `start` command.
